...
func NewGateRepository(pool *pgxpool.Pool) *GateRepository {
	return &GateRepository{base{pool: pool}}
}

func (r *GateRepository) CountGates(ctx context.Context) (int64, error) {
	row := r.conn(ctx).QueryRow(ctx, `SELECT count(*) FROM gates`)
	var count int64
	err := row.Scan(&count)
...
func (r *GateRepository) DeleteGate(ctx context.Context, id string) error {
	...
	row := r.conn(ctx).QueryRow(ctx, `DELETE FROM gates WHERE id = $1 RETURNING id`, uid)
